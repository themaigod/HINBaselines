...
all_edges, node_type_features, labels, train_idx, val_idx, test_idx = load_datasets("../data/IMDB",
...
model = HGT(G,
            ...
            n_layers=4,
            n_heads=8,
            use_norm=True, dropout=0.35).to(device)
optimizer = torch.optim.AdamW(model.parameters())
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 1000, eta_min=1e-6)
print('Training HGT with #param: %d' % (get_n_params(model)))
train(model, G)

all_edges, node_type_features, labels, train_idx, val_idx, test_idx = load_datasets("../data/IMDB", labels_path=args.label)
G = dgl.heterograph(all_edges)
print(G)
