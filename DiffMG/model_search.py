...
class Model(nn.Module):
    ...
    def __init__(self, in_dim, n_hid, num_node_types, n_adjs, n_classes, n_steps, cstr, attn_dim=64, use_norm=True,
                 out_nl=True):
        ...
        if np.random.uniform() < eps:
            for i in range(len(self.metas)):
                ...
                temp.append(None if self.as_seq[i] is None else torch.randint(low=0, high=self.as_seq[i].size(-1),
                                                                              size=self.as_seq[i].size()[:-1]).cuda())
