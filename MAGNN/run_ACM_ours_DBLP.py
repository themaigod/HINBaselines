...
from load_dataset import process_acm_2_dblp_like
from utils.tools import index_generator, evaluate_results_nc, parse_minibatch, score
...
def run_model_DBLP(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
                   num_epochs, patience, batch_size, neighbor_samples, repeat, save_postfix, labels_path, device):
    adjlists, edge_metapath_indices_list, features_list, adjM, type_mask, labels, train_val_test_idx = process_acm_2_dblp_like(
        r"../data/ACM", labels_path)
    ...
    for _ in range(repeat):
        net = MAGNN_nc_mb(2, 4, etypes_list, in_dims, hidden_dim, out_dim, num_heads, attn_vec_dim, rnn_type,
                          dropout_rate)
        net.to(device)
        ...
        with torch.no_grad():
            ...
            acc_test, micro_f1_test, macro_f1_test = score(test_logits, labels[test_idx])
            print("direct test summary, micro_f1: {}, macro_f1: {}".format(micro_f1_test, macro_f1_test))
...
if __name__ == '__main__':
    ...
    ap.add_argument("--label", type=str, default="../data/ACM/labels_5_fold_cross_validation_4.pkl")
    ap.add_argument('--result', type=str, default="./")
    ...
    run_model_DBLP(args.feats_type, args.hidden_dim, args.num_heads, args.attn_vec_dim, args.rnn_type,
                   args.epoch, args.patience, args.batch_size, args.samples, args.repeat, args.save_postfix, args.label, device)
