...
from load_dataset import process_acm

from utils.pytorchtools import EarlyStopping
from utils.data import load_IMDB_data
from utils.tools import evaluate_results_nc, score
...
def run_model_ACM(feats_type, num_layers, hidden_dim, num_heads, attn_vec_dim, rnn_type,
                   num_epochs, patience, repeat, save_postfix):
    nx_G_lists, edge_metapath_indices_lists, features_list, adjM, type_mask, labels, train_val_test_idx = process_acm(
        r"../data/ACM")
    ...
    if feats_type == 0:
        ...
        for i in range(len(features_list)):
            ...
            values = torch.FloatTensor(np.ones(dim))
            features_list[i] = torch.sparse.FloatTensor(indices, values, torch.Size([dim, dim])).to(device)
    edge_metapath_indices_lists = [[torch.LongTensor(indices).to(device) for indices in indices_list] for indices_list
                                   in
    ...
    for _ in range(repeat):
        net = MAGNN_nc(num_layers, [2], 4, etypes_lists, in_dims, hidden_dim, out_dim, num_heads, attn_vec_dim,
        ...
        for epoch in range(num_epochs):
            ...
            # training forward
            net.train()
            logits, embeddings = net((g_lists, features_list, type_mask, edge_metapath_indices_lists),
                                     target_node_indices)
        ...
        with torch.no_grad():
            ...
            acc_test, micro_f1_test, macro_f1_test = score(test_logits[test_idx], labels[test_idx])
...
if __name__ == '__main__':
    ...
    args = ap.parse_args()
    run_model_ACM(args.feats_type, args.layers, args.hidden_dim, args.num_heads, args.attn_vec_dim, args.rnn_type,
